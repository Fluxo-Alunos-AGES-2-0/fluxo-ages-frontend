...
export const InputField: React.FC<InputFieldProps> = ({
  ...
  let wrapperClasses = `flex items-center border rounded-lg px-3 py-2.5 transition-all duration-200 ease-in-out ${
    disabled
      ? "bg-[#f4f5f6] dark:bg-[#334155] border-[#e2e4e9] dark:border-[#334155]"
      : error
        ? "border-red-500 bg-white dark:bg-[#1E293B]"
        : isFocused
          ? "border-blue-500 ring-1 ring-blue-500 bg-white dark:bg-[#1E293B]"
          : "border-slate-300 dark:border-[#334155] bg-white dark:bg-[#1E293B]"
  }`;
  ...
  return (
    <div className={`flex flex-col w-full font-sans ${className}`}>
      ...
      <div className={wrapperClasses}>
        ...
        {icon && (
          <div
            className={`flex items-center mr-2.5 ${disabled ? "text-slate-300 dark:text-[#64748B]" : "text-slate-400 dark:text-[#94A3B8]"}`}
          >
            {icon}
          </div>
        )}
        ...
        <input
          ...
          className={`flex-1 border-none outline-none bg-transparent text-sm disabled:cursor-not-allowed ${
            disabled
              ? "text-slate-400 dark:text-[#94A3B8] placeholder-slate-300 dark:placeholder-[#64748B]"
              : "text-slate-700 dark:text-[#F4F6F7] placeholder-slate-400 dark:placeholder-[#64748B]"
          }`}
